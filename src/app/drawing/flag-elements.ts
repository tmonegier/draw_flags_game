export type ElementCategory = 'maps' | 'coat_of_arms' | 'animals' | 'plants' | 'symbols';

export interface AutoPlace {
  /** Horizontal center as a fraction of canvas width (0–1) */
  xCenter: number;
  /** Vertical center as a fraction of canvas height (0–1) */
  yCenter: number;
  /** Element size as a fraction of canvas height */
  sizeFraction: number;
}

export interface FlagElement {
  id: string;
  name: string;
  /** Country or flag this element comes from */
  flagOf: string;
  category: ElementCategory;
  /** Suggested hex color as used on the real flag */
  defaultColor: string;
  /** SVG shape elements for viewBox="0 0 100 100". Uses currentColor for fill/stroke. */
  svgContent: string;
  /**
   * When present, the element is stamped automatically at this position/size
   * instead of entering interactive placement mode.
   */
  autoPlace?: AutoPlace;
}

// ── Slovenia coat of arms ─────────────────────────────────────────────────────
// Paths are in the local coordinate space of the original <g> element (before
// the matrix(.391 0 0 .391 60 60) transform). All colors hardcoded — no currentColor.
const SLOVENIA_SHIELD_PATH = `M-37.175342-94.368205a92.195445 92.195445 0 0 1 74.350684 0Q43-12 0-1q-43-11-37.175342-93.368205Z`;
const SLOVENIA_STAR_PATH   = `m0-84.5 1.125 2.551443L3.897114-82.25 2.25-80l1.647114 2.25L1.125-78.051443 0-75.5l-1.125-2.551443-2.772114.301443L-2.25-80l-1.647114-2.25 2.772114.301443`;
const SLOVENIA_MOUNTAIN_PATH = `m0-70 9 18 6-8 15 20a10 10 0 0 1-.384761 2.747211A46.400549 46.400549 0 0 1 0-6.090878a46.400549 46.400549 0 0 1-29.615239-31.161911A10 10 0 0 1-30-40l15-20 6 8z`;
const SLOVENIA_WAVE_PATH   = `M-29.615239-37.252789A10 10 0 0 0-15-31.339746a10 10 0 0 1 10 0 10 10 0 0 0 10 0 10 10 0 0 1 10 0 10 10 0 0 0 14.615239-5.913043L30-37.113249v7.320508a10 10 0 0 0-5 1.339746 10 10 0 0 1-10 0 10 10 0 0 0-10 0 10 10 0 0 1-10 0 10 10 0 0 0-10 0 10 10 0 0 1-10 0 10 10 0 0 0-5-1.339746v-7.320508`;
const SLOVENIA_BORDER_PATH = `M-40-93.066239a92.195445 92.195445 0 0 1 2.824658-1.301966l2.97064 47.448778A49.301041 49.301041 0 0 0 0-3.036262a49.301041 49.301041 0 0 0 34.204702-43.883164l2.97064-47.448778A92.195445 92.195445 0 0 1 40-93.066239L37.099526-46.73819A52.201533 52.201533 0 0 1 0 0a52.201533 52.201533 0 0 1-37.099526-46.738189Z`;

// ── Slovakia coat of arms ─────────────────────────────────────────────────────
// Paths are in the original 900×600 flag coordinate space. All colors hardcoded.
const SLOVAKIA_SHIELD_OUTLINE = `m269.993 459.98-3.906-1.867c-25.267-12.173-56.294-30.4-81.294-58.133-25-27.733-43.8-65.307-43.8-114.24 0-93.6 4.52-136.68 4.52-136.68l.84-8.067h247.28l.84 8.067s4.534 43.093 4.534 136.68c0 48.933-18.8 86.507-43.814 114.24-25 27.733-56.026 45.96-81.293 58.133Z`;
const SLOVAKIA_RED_FILL      = `M270 450c-49.38-23.76-120-70.94-120-164.25S154.46 150 154.46 150h231.07S390 192.44 390 285.75 319.37 426.24 270 450`;
const SLOVAKIA_CROSS_PATH    = `M280.56 261.28c13.36.22 39.45.74 62.67-7.03 0 0-.61 8.31-.61 17.99 0 9.67.61 17.98.61 17.98-21.3-7.12-47.61-7.27-62.67-7.08v51.54h-21.12v-51.54c-15.07-.2-41.37-.04-62.68 7.08 0 0 .62-8.3.62-17.98s-.62-17.99-.62-17.99c23.23 7.77 49.31 7.25 62.68 7.03v-32.37c-12.19-.1-29.74.48-49.6 7.12 0 0 .62-8.3.62-17.98s-.62-17.98-.62-17.98c19.83 6.62 37.36 7.22 49.54 7.11-.62-20.5-6.6-46.33-6.6-46.33s12.3.96 17.22.96c4.92 0 17.21-.96 17.21-.96s-5.97 25.83-6.6 46.33c12.18.1 29.72-.49 49.55-7.11 0 0-.62 8.3-.62 17.98 0 9.67.62 17.98.62 17.98-19.86-6.64-37.42-7.22-49.6-7.12v32.37`;
const SLOVAKIA_MOUNTAIN_PATH = `M270 329.1c-24.87 0-38.19 34.46-38.19 34.46s-7.4-16.34-27.68-16.34c-13.73 0-23.82 12.2-30.25 23.5 24.97 39.7 64.8 64.2 96.11 79.28 31.32-15.07 71.16-39.58 96.13-79.28-6.43-11.3-16.52-23.5-30.25-23.5a30.52 30.52 0 0 0-27.69 16.34s-13.32-34.46-38.19-34.46Z`;

// ── Morocco pentagram ─────────────────────────────────────────────────────────
// Stroked outline only (fill="none"). Coordinate space: 90000×60000.
// Star tip at (45000,17308); bounding box ≈ x:32929–57071, y:17308–40268.
const MOROCCO_STAR_PATH = `m45000 17308 7460 22960-19531-14190h24142L37540 40268z`;

// ── Algeria crescent + star ───────────────────────────────────────────────────
// Single path in the 900×600 flag coordinate space.
// Crescent: outer circle center (450,300) r=150; inner offset right, r=120.
// Star: tips span x≈450–586, y≈229–371 — centered near (511,300).
const ALGERIA_CRESCENT_STAR_PATH = `M579.903811 225a150 150 0 1 0 0 150 120 120 0 1 1 0-150M585.676275 300 450 255.916106 533.852549 371.329239v-142.658277L450 344.083894z`;

// ── Turkey crescent + star ────────────────────────────────────────────────────
// Single path in the 90000×60000 space (viewBox "0 -30000 90000 60000", y=0 = vert centre).
// Crescent outer circle: centre (30000,0) r=15000; inner: centre (33750,0) r=12000.
// Star centre: ≈(49250,0). Both shapes use fill="currentColor".
const TURKEY_CRESCENT_STAR_PATH = `m41750 0 13568-4408-8386 11541V-7133l8386 11541zm925 8021a15000 15000 0 1 1 0-16042 12000 12000 0 1 0 0 16042z`;

// ── Maple leaf path from the Canadian flag SVG (viewBox 9600×4800 coordinate space).
// The path starts at (2490, 4430) when used as the first command (absolute moveto).
const CANADA_MAPLE_LEAF_PATH = `m2490 4430-45-863a95 95 0 0 1 111-98l859 151-116-320a65 65 0 0 1 20-73l941-762-212-99a65 65 0 0 1-34-79l186-572-542 115a65 65 0 0 1-73-38l-105-247-423 454a65 65 0 0 1-111-57l204-1052-327 189a65 65 0 0 1-91-27l-332-652-332 652a65 65 0 0 1-91 27l-327-189 204 1052a65 65 0 0 1-111 57l-423-454-105 247a65 65 0 0 1-73 38l-542-115 186 572a65 65 0 0 1-34 79l-212 99 941 762a65 65 0 0 1 20 73l-116 320 859-151a95 95 0 0 1 111 98l-45 863z`;

// Eagle path data shared by both halves of the Albanian double-headed eagle.
// Original coordinate space: 700×500. Both paths use the same d attribute;
// the right half is rendered with transform="matrix(-1,0,0,1,700,0)".
const ALBANIA_EAGLE_PATH = `m300.094,97.2043c-4.8618-0.082-12.9308,1.54998-12.75,5.2812-13.5419-2.32092-14.952,3.27267-14.1562,8.25003,1.28844-1.97532,2.85723-3.04776,4.06251-3.24999,1.80576-0.2997,3.6954,0.2835,5.625,1.46871,1.92969,1.1853,4.05558,3.09798,5.00004,4.28121-4.77945,1.14264-8.4717,0.405-12.2188-0.2502-1.84716-0.3231-4.43205-1.36224-5.96871-2.43747-1.53666-1.07541-2.0205-2.08755-4.43754-4.56246-2.84967-2.91798-5.88258-2.09637-4.93749,2.43747,2.18556,4.21092,5.83659,6.08859,10.4375,6.84378,2.214,0.3636,5.51196,1.15263,9.25,1.15623,3.73806,0.004,7.93521-0.54,10.2188-0.0625-1.37187,0.8613-2.92428,2.34621-6.00003,2.93751-3.12777,0.6012-7.88301-1.85994-10.7812-2.53125,0.3681,2.43855,3.44403,4.71816,9.49999,5.90625,9.95152,2.16693,18.2208,3.81375,23.6875,6.78123,5.46669,2.96748,8.91342,6.68268,11.375,9.59375,4.89861,5.79312,5.15628,10.2402,5.46876,11.2187,1.00782,9.2557-2.21958,14.4622-8.21871,16.0625-2.99961,0.8001-8.32788-0.7065-10.2812-2.99997-1.95345-2.29293-3.85407-6.23421-3.31254-12.4062,0.5265-2.42127,3.29526-8.72811,0.93753-10.0312-10.8616-6.00326-24.0512-12.0744-33.5938-15.6873-2.60815-0.98745-4.75675,2.55735-5.59375,3.9375-16.2111-1.97418-30.827-12.9443-37.4062-24.625-4.48101-7.95555-11.8665,0.0164-10.625,7.5312,2.0043,8.38764,8.39502,14.4519,16.0625,18.75,7.86951,4.33314,17.7059,8.59698,27.6562,8.37504,5.36067,1.01412,5.30775,7.94943-1.125,9.24999-12.5809,0.0796-22.6175-0.234-32.0938-9.375-7.18776-6.5628-11.2342,1.25694-9.15624,5.68746,3.52134,13.6583,23.0063,17.483,42.7187,13.0625,7.63344-1.26414,3.06252,6.91542,0.90621,7.00002-8.23662,5.90562-22.9827,11.6841-35.9688-0.0625-5.94243-4.55022-9.96016-0.7254-7.74999,5.84379,5.76216,17.1274,27.8037,13.5302,42.9375,5.09373,3.89529-2.17143,7.43049,2.88045,2.68749,6.71877-18.8201,13.1503-28.2298,13.2999-36.7187,8.24998-10.6275-4.1913-11.5623,7.59637-5.25,11.4688,7.01595,4.30398,24.8701,1.07703,37.9688-7.15626,5.61537-4.17483,5.86944,2.35305,2.31246,4.93749-15.5442,13.4334-21.671,16.971-37.875,14.7812-8.03511-0.6264-7.91442,9.2799-1.59372,13.1562,8.62983,5.29245,25.4827-3.49245,38.5625-14.3438,5.50521-2.94165,6.40935,1.88271,3.68748,7.59375-7.99209,10.0832-15.4932,16.0281-22.7187,18.7812-7.22556,2.75319-14.1755,2.31453-19.0938,0.6246-6.00021-2.06145-6.75261,4.17429-3.43746,9.84375,1.99836,3.41748,10.2797,4.51332,19.2187,1.34379,8.93907-3.16908,18.5358-10.6026,25.125-19.3125,5.73291-5.01894,5.01993,1.72368,2.40624,6.46875-13.1944,20.8587-25.262,28.5963-41.1562,27.2812-7.04754-1.19673-8.6481,4.28364-4.1562,9.34375,7.88742,6.54111,17.7493,6.336,26.375-0.1251,7.66818-7.39323,22.3379-23.3134,30.0312-31.8438,5.42133-4.32189,7.1532-0.0612,5.56254,8.71875-1.44666,5.02803-5.06826,10.3234-14.9375,14.1875-6.74226,3.84804-1.67904,9.15142,3.375,9.25,2.78136,0.0542,8.42922-3.19716,12.75-8.06247,5.68422-6.40053,6.02226-10.6976,9.15625-20.6562,2.96091-4.85442,8.24949-2.58858,8.25003,2.50002-2.53647,9.97519-4.71582,11.7638-9.84375,15.8438-4.90482,4.60494,3.41523,6.13962,6.21873,4.24998,8.11044-5.46678,11.0713-12.5383,13.7812-18.9688,1.95606-4.64166,7.63038-2.39148,5.00004,5.18751-6.28605,18.1128-16.6152,25.2447-34.6875,28.9062-1.83159,0.3258-2.94876,1.40661-2.31246,3.53124,2.42559,2.51811,4.85559,4.80303,7.28127,7.3125-11.1751,3.25377-20.2541,5.08171-31.4377,8.34421-5.48685-3.59721-9.9194-6.64909-15.4062-10.25-1.44684-3.38229-2.12149-8.54505-10.2188-4.90626-5.48685-2.53413-8.01783-1.60488-11.0625,0.93753,4.39542,0.1638,6.30783,1.34091,8.03124,3.34377,2.25009,5.92614,7.43877,6.49863,12.75,4.81248,3.45528,2.91078,5.29542,5.1435,8.74998,8.06247-6.44922-0.2205-10.9422-0.3348-17.375-0.531-6.1407-6.5934-11.042-6.23223-15.4375-1.06245-3.35007,0.5148-4.77018,0.5868-7.06248,4.65624,3.60513-1.4796,5.87322-1.92312,7.43751-0.3123,6.52869,3.78495,10.7938,2.94012,14.0313,0,6.29874,0.3852,11.9746,0.711,18.2812,1.125-2.3166,1.96911-5.49585,3.01365-7.81254,5.00004-9.4603-2.70648-14.3979,0.93663-16.0625,8.65629-1.26783,3.11571-1.86147,6.32349-1.31247,9.65626,0.91566-3.04839,2.38788-5.66937,5.09373-7.3125,8.43201,2.14182,11.6026-1.3005,12.0625-6.34374,4.06449-3.31632,10.1935-4.04766,14.25-7.40628,4.743,1.51839,7.03575,2.45889,11.8125,3.96873,1.6983,5.16078,5.5413,7.20423,11.7812,5.87502,7.42797,0.2331,6.11667,3.28095,6.71877,5.71878,1.97406-3.50109,1.91907-6.90588-2.65626-10-1.66464-4.52241-5.35275-6.57918-10.1875-3.96873-4.55094-1.29204-5.7474-3.1491-10.2812-4.43754,11.4681-3.65526,19.603-4.47777,31.0625-8.12502,2.86848,2.70585,5.14188,4.64841,8.03124,7.06248,1.5228,0.90891,2.98053,1.13787,3.87504,0,7.18083-10.3932,10.3879-19.5528,17.0625-26.4062,2.54997-2.83554,5.77161-6.66135,9.34375-7.59375,1.78605-0.4662,3.97638-0.1818,5.37498,1.34379,1.3986,1.52541,2.49723,4.33683,2.0313,8.53128-0.6687,6.02082-2.12337,7.9218-3.81249,11.5312-1.68912,3.60954-3.75336,5.82975-5.87502,8.59374-4.24341,5.52789-9.8424,8.74539-13.1562,10.9062-6.62769,4.3218-9.43073,2.43027-14.5625,2.15622-6.63273,0.7443-8.39754,3.97467-2.96874,8.4375,5.07492,2.64051,9.63652,2.96649,13.3438,2.28123,3.18312-0.5886,6.90831-4.6971,9.5625-6.90624,2.98764-3.4515,7.94097,0.6417,4.56246,4.65624-6.13935,7.29477-12.2738,12.1044-19.8438,12-7.95465,1.07055-6.46668,5.54004-1.18746,7.71876,9.50117,3.92121,18.0796-3.42351,22.4375-8.25003,3.36222-3.67839,5.75028-3.82185,5.15628,1.87497-3.33783,10.3136-7.89912,14.2977-15.3438,14.8125-6.03855-0.5607-6.1038,4.10058-1.6875,7.25004,10.0603,6.96321,17.3459-4.89474,20.75-12.0625,2.42568-6.46569,6.14619-3.39093,6.5313,1.93752,0.0502,7.11936-3.16674,12.9321-11.7812,20.2187,6.59115,10.5015,14.2752,21.1829,20.8749,31.7188l20.005-222.806-20-35.202c-2.08305-1.92312-9.12777-10.223-10.9686-11.375-0.6714-0.7173-1.07973-1.22409-0.0936-1.59372,0.95364-0.3573,3.18591-0.7578,4.68747-1.03122-4.2417-4.2489-7.87662-5.61159-15.9065-7.93728,1.95687-0.8334,3.86037-0.3483,9.625-0.6246-2.2886-3.24868-7.40028-8.22482-14-10.6254,4.35852-3.09852,5.20785-3.30617,9.53126-6.93749-7.48656-0.531-13.8798-1.95712-20.3125-3.90625-4.06692-1.90161-9.71539-3.51615-12.4687-3.56251zm0.7191,8.71875c3.95622,0,6.40629,1.35612,6.40629,2.99997,0,1.67238-2.45007,3.03129-6.40629,3.03129-3.93975,0-6.43752-1.48293-6.43752-3.15621,0.00071-1.64385,2.49777-2.87505,6.43752-2.87505z`;

export const FLAG_ELEMENTS: FlagElement[] = [
  // ── Symbols: moon / star ────────────────────────────────────────────────────
  {
    id: 'morocco-star',
    name: 'Star',
    flagOf: 'Morocco',
    category: 'symbols',
    defaultColor: '#006233',
    // 90000×60000 coordinate space. Stroke-only pentagram; stroke-width=1426.
    svgContent: `<svg viewBox="30000 14000 30000 30000" width="100" height="100" xmlns="http://www.w3.org/2000/svg"><path fill="none" stroke="currentColor" stroke-width="1426" d="${MOROCCO_STAR_PATH}"/></svg>`,
    autoPlace: { xCenter: 0.5, yCenter: 0.5, sizeFraction: 0.45 },
  },
  {
    id: 'algeria-crescent-star',
    name: 'Crescent and Star',
    flagOf: 'Algeria',
    category: 'symbols',
    defaultColor: '#d21034',
    // 900×600 coordinate space. Crescent opens right; star is inside the opening.
    // ViewBox centred on the element bounding box (x:295–586, y:150–450).
    svgContent: `<svg viewBox="275 135 335 335" width="100" height="100" xmlns="http://www.w3.org/2000/svg"><path fill="currentColor" d="${ALGERIA_CRESCENT_STAR_PATH}"/></svg>`,
    autoPlace: { xCenter: 0.5, yCenter: 0.5, sizeFraction: 0.55 },
  },
  {
    id: 'tunisia-crescent-star',
    name: 'Crescent and Star',
    flagOf: 'Tunisia',
    category: 'symbols',
    defaultColor: '#e70013',
    // ViewBox="-60 -40 120 80" coordinate space (origin = flag centre).
    // Multi-colour emblem: white disc + red (currentColor) crescent + red star.
    svgContent: `<svg viewBox="-25 -25 50 50" width="100" height="100" xmlns="http://www.w3.org/2000/svg"><circle fill="#ffffff" r="20"/><circle fill="currentColor" r="15"/><circle fill="#ffffff" cx="4" r="12"/><path fill="currentColor" d="M-5 0l16.281-5.29L1.22 8.56V-8.56L11.28 5.29z"/></svg>`,
    autoPlace: { xCenter: 0.5, yCenter: 0.5, sizeFraction: 0.5 },
  },
  {
    id: 'turkey-crescent-star',
    name: 'Crescent and Star',
    flagOf: 'Turkey',
    category: 'symbols',
    defaultColor: '#ffffff',
    // 90000×60000 space, y=0 is vertical centre (viewBox "0 -30000 90000 60000").
    // Crescent outer circle: centre (30000,0) r=15000. Star centre ≈(49250,0).
    svgContent: `<svg viewBox="13000 -17000 45000 34000" width="100" height="100" xmlns="http://www.w3.org/2000/svg"><path fill="currentColor" d="${TURKEY_CRESCENT_STAR_PATH}"/></svg>`,
    autoPlace: { xCenter: 0.38, yCenter: 0.5, sizeFraction: 0.55 },
  },
  // ── Coat of arms ────────────────────────────────────────────────────────────
  {
    id: 'slovenia-coat-of-arms',
    name: 'Coat of Arms',
    flagOf: 'Slovenia',
    category: 'coat_of_arms',
    defaultColor: '#0000ff',
    // Local coordinate space (before the original flag transform).
    // Bounds: x ≈ [-43, 43], y ≈ [-99, 0]. Stars poke above the shield top.
    svgContent: `<svg viewBox="-48 -105 96 112" width="100" height="100" xmlns="http://www.w3.org/2000/svg"><path fill="#0000ff" d="${SLOVENIA_SHIELD_PATH}"/><path fill="#ffff00" d="${SLOVENIA_STAR_PATH}"/><g transform="translate(-10.5,-14)"><path fill="#ffff00" d="${SLOVENIA_STAR_PATH}"/></g><g transform="translate(10.5,-14)"><path fill="#ffff00" d="${SLOVENIA_STAR_PATH}"/></g><path fill="#ffffff" d="${SLOVENIA_MOUNTAIN_PATH}"/><path fill="#0000ff" d="${SLOVENIA_WAVE_PATH}"/><g transform="translate(0,5.7735028)"><path fill="#0000ff" d="${SLOVENIA_WAVE_PATH}"/></g><path fill="#ff0000" d="${SLOVENIA_BORDER_PATH}"/></svg>`,
    // In the 240×120 flag space the g is at (60,60); coat-of-arms spans y≈23–60 → center ≈ (60, 41.5)
    autoPlace: { xCenter: 0.25, yCenter: 0.346, sizeFraction: 0.35 },
  },
  {
    id: 'slovakia-coat-of-arms',
    name: 'Coat of Arms',
    flagOf: 'Slovakia',
    category: 'coat_of_arms',
    defaultColor: '#ed1c24',
    // Paths in the original 900×600 flag coordinate space.
    // Bounds: x ≈ [141, 399], y ≈ [141, 460].
    svgContent: `<svg viewBox="130 130 285 345" width="100" height="100" xmlns="http://www.w3.org/2000/svg"><path fill="#ffffff" d="${SLOVAKIA_SHIELD_OUTLINE}"/><path fill="#ed1c24" d="${SLOVAKIA_RED_FILL}"/><path fill="#ffffff" d="${SLOVAKIA_CROSS_PATH}"/><path fill="#254aa5" d="${SLOVAKIA_MOUNTAIN_PATH}"/></svg>`,
    // Shield center ≈ (270, 300) in the 900×600 flag → fractions (0.3, 0.5)
    autoPlace: { xCenter: 0.3, yCenter: 0.5, sizeFraction: 0.52 },
  },
  {
    id: 'canada-maple-leaf',
    name: 'Maple Leaf',
    flagOf: 'Canada',
    category: 'plants',
    defaultColor: '#d52b1e',
    // ViewBox frames the leaf bounds (x≈540–4260, y≈400–4430) with padding.
    svgContent: `<svg viewBox="400 280 4000 4260" width="100" height="100" xmlns="http://www.w3.org/2000/svg"><path d="${CANADA_MAPLE_LEAF_PATH}" fill="currentColor"/></svg>`,
    autoPlace: { xCenter: 0.5, yCenter: 0.5, sizeFraction: 0.88 },
  },
  {
    id: 'albania-eagle',
    name: 'Double-headed Eagle',
    flagOf: 'Albania',
    category: 'coat_of_arms',
    defaultColor: '#000000',
    // Nested SVG crops the eagle from the 700×500 original coordinate space.
    // Both halves use the same path; the right half is mirrored at x=350.
    svgContent: `<svg viewBox="155 80 390 390" width="100" height="100" xmlns="http://www.w3.org/2000/svg"><path d="${ALBANIA_EAGLE_PATH}" fill="currentColor"/><path d="${ALBANIA_EAGLE_PATH}" fill="currentColor" transform="matrix(-1,0,0,1,700,0)"/></svg>`,
    autoPlace: { xCenter: 0.5, yCenter: 0.55, sizeFraction: 0.78 },
  },
];
